...
export const AdminDashboardView: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (prodsData.status === "fulfilled" && prodsData.value.length > 0) {
        setLowStockProducts(prodsData.value.filter((p) => p.stock < (p.low_stock_threshold || 100)));
      } else {
        setLowStockProducts(INITIAL_PRODUCTS.filter((p: any) => p.stock < (p.low_stock_threshold || 100)) as any);
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Banner */}
      <div className="bg-[#0b2341] text-white rounded-[6px] p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <span className="bg-[#A71380]/30 text-[#F3D0E9] border border-[#F3D0E9]/40 px-3 py-1 rounded-[4px] text-[10px] font-extrabold uppercase tracking-wider">
        ...
        <div className="flex items-center space-x-3 w-full md:w-auto">
          <Link
            href="/admin/products"
            className="bg-[#A71380] hover:bg-[#8E0F6D] text-white text-xs font-extrabold px-4 py-2.5 rounded-[5px] shadow-sm shadow-[#A71380]/30 transition-all flex items-center space-x-2 shrink-0 cursor-pointer"
          >
          ...
          return (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 items-stretch">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="bg-white/60 backdrop-blur-md border border-white/80 rounded-[8px] p-5 shadow-lg shadow-slate-200/50 flex flex-col justify-between h-full min-h-[145px] relative overflow-hidden animate-pulse"
                >
                  <div className="flex items-center justify-between">
                    <div className="h-3 w-28 bg-slate-200/80 rounded-full" />
                    <div className="w-9 h-9 rounded-[6px] bg-slate-200/80 shrink-0" />
                  </div>
                  <div className="my-2 space-y-1.5">
                    <div className="h-8 w-36 bg-slate-300/80 rounded-md" />
                  </div>
                  <div className="flex items-center justify-between pt-1">
                    <div className="h-3 w-24 bg-slate-200/80 rounded-full" />
                    <div className="h-3 w-10 bg-purple-200/70 rounded-full" />
                  </div>
                </div>
              ))}
            </div>
        ...
        return (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 items-stretch">
            ...
            <Link href="/admin/orders" className="block h-full group">
              <div className="bg-[#f7f6f4] border border-[#e8e6e2] rounded-[6px] p-5 shadow-2xs flex flex-col justify-between h-full min-h-[145px] hover:shadow-xs hover:border-[#0b2341]/30 transition-all cursor-pointer">
                <div className="flex items-center justify-between">
                ...
                <div className="text-3xl font-extrabold text-[#0b2341] tracking-tight my-1">
                  {summary ? (summary.total_orders ?? (summary.orders_pending + summary.orders_delivered)) : "0"}
                </div>
                <div className="text-[11px] font-bold text-emerald-700 flex items-center justify-between">
                  ...
                  <span className="text-[#A71380] font-extrabold opacity-0 group-hover:opacity-100 transition-opacity text-[10px]">View All &rarr;</span>
                </div>
              </div>
            </Link>

            {/* Card 2: Cumulative Realized Revenue */}
            <Link href="/admin/reports" className="block h-full group">
              <div className="bg-[#f7f6f4] border border-[#e8e6e2] rounded-[6px] p-5 shadow-2xs flex flex-col justify-between h-full min-h-[145px] hover:shadow-xs hover:border-[#0b2341]/30 transition-all cursor-pointer">
                <div className="flex items-center justify-between">
                ...
                <div className="text-3xl font-extrabold text-[#0b2341] tracking-tight my-1">
                  ₹{summary ? summary.total_sales_all_time.toLocaleString('en-IN') : "0"}
                </div>
                <div className="text-[11px] font-bold flex flex-wrap items-center justify-between gap-1.5">
                  <span className="text-emerald-700">
                    ₹{summary ? summary.total_sales_today.toLocaleString('en-IN') : "0"} paid today
                  </span>
                  {pendingCreditToday > 0 && (
                    <span className="bg-[#F8EAF4] text-[#A71380] text-[10px] px-1.5 py-0.5 rounded-[4px] font-extrabold">
                      + ₹{pendingCreditToday.toLocaleString('en-IN')} credit/COD
                    </span>
                  )}
                  <span className="text-[#A71380] font-extrabold opacity-0 group-hover:opacity-100 transition-opacity text-[10px]">Reports &rarr;</span>
                </div>
              </div>
            </Link>

            {/* Card 3: Pending KYC Approvals */}
            <Link href="/admin/kyc" className="block h-full group">
              <div className="bg-[#f7f6f4] border border-[#e8e6e2] rounded-[6px] p-5 shadow-2xs flex flex-col justify-between h-full min-h-[145px] hover:shadow-xs hover:border-amber-300 transition-all cursor-pointer">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500 block">Pending KYC Approvals</span>
                  <div className="w-9 h-9 rounded-[5px] bg-amber-100 border border-amber-200 text-amber-800 flex items-center justify-center shadow-xs shrink-0 group-hover:bg-amber-500 group-hover:text-white transition-colors">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="text-[11px] font-extrabold text-[#A71380] group-hover:text-[#8E0F6D] flex items-center justify-between">
            ...
            <Link href="/admin/products" className="block h-full group">
              <div className="bg-[#f7f6f4] border border-[#e8e6e2] rounded-[6px] p-5 shadow-2xs flex flex-col justify-between h-full min-h-[145px] hover:shadow-xs hover:border-[#0b2341]/30 transition-all cursor-pointer">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500 block">Active Products</span>
                  <div className="w-9 h-9 rounded-[5px] bg-[#0b2341] text-white flex items-center justify-center shadow-xs shrink-0 group-hover:bg-[#A71380] transition-colors">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="text-[11px] font-bold text-slate-600 flex items-center justify-between">
                  <span>{summary ? `${summary.low_stock_count} low stock` : "0 low stock"}</span>
                  <span className="text-[#A71380] font-extrabold opacity-0 group-hover:opacity-100 transition-opacity text-[10px]">Products &rarr;</span>
                </div>
              </div>
            </Link>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200/90 rounded-[6px] p-6 shadow-2xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <Link href="/admin/orders" className="text-xs font-extrabold text-[#A71380] hover:text-[#8E0F6D] hover:underline transition-colors">
                ...
                {(() => {
                  ...
                  return filtered.map((ord: any) => (
                    <tr key={ord.id || ord.order_code} className="hover:bg-slate-50/80 transition-colors">
                      <td className="py-3 px-3 font-mono font-bold text-[#A71380]">
                        <Link href={`/admin/orders/${ord.id || ord.order_code}`} className="hover:underline hover:text-[#8E0F6D]">
                          {ord.order_code || ord.id}
                        </Link>
                      </td>
                      <td className="py-3 px-3 font-semibold text-slate-800">{ord.customer_name || ord.customerName}</td>
                      <td className="py-3 px-3">
                        <span className={`px-2 py-0.5 rounded-[4px] text-[10px] font-bold ${(ord.role || "").includes("Distributor") ? "bg-emerald-50 text-emerald-800 border border-emerald-200" : "bg-[#F8EAF4] text-[#A71380] border border-[#F3D0E9]"
                          }`}>
                          {ord.role}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-3">
                        <span className={`px-2.5 py-0.5 rounded-[4px] font-bold text-[10px] uppercase ${(ord.order_status || ord.orderStatus) === "Delivered" ? "bg-emerald-100 text-emerald-800" :
                          (ord.order_status || ord.orderStatus) === "Shipped" ? "bg-purple-100 text-purple-800" :
                            (ord.order_status || ord.orderStatus) === "Confirmed" ? "bg-[#F8EAF4] text-[#A71380] border border-[#F3D0E9]" :
        ...
        <div className="bg-white border border-slate-200/90 rounded-[6px] p-6 shadow-2xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <Link href="/admin/inventory" className="text-xs font-extrabold text-[#A71380] hover:text-[#8E0F6D] hover:underline transition-colors">
              Manage &rarr;
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {(() => {
              ...
              return filteredProds.map((p) => (
                <div key={p.id || p.sku} className="p-3 rounded-[5px] bg-slate-50 border border-slate-100 flex items-center justify-between text-xs">
                  <div>
