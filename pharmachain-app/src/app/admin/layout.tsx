...
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  // Extract active tab from URL path and query parameters
  const subPath = pathname.replace(/^\/admin\/?/, "");
  const statusParam = searchParams.get("status");
  const activeTab =
    statusParam && subPath.startsWith("orders")
      ? `orders?status=${statusParam}`
      : subPath || "dashboard";
        ...
        <header className="bg-white border-b border-slate-200/80 py-3.5 px-4 md:px-8 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="md:hidden p-2 rounded-[5px] border border-slate-200 text-[#0b2341] hover:bg-slate-50 transition-colors"
          ...
          <div className="flex items-center space-x-4">
            <div className="relative hidden sm:block">
              <input
                ...
                className="bg-slate-50 border border-slate-200 text-xs text-slate-800 rounded-[5px] pl-9 pr-8 py-2 w-56 md:w-72 focus:outline-none focus:bg-white focus:border-[#A71380] transition-all font-medium"
