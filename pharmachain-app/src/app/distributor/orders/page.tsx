      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/90 shadow-2xs">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Track real-time shipment status, AWB tracking numbers, itemized formulations, and download invoices.
          </p>
        </div>

        <Link
          href="/distributor/orders/new"
          className="bg-[#0b2341] hover:bg-[#12315a] text-white text-xs font-extrabold px-5 py-3 rounded-xl shadow-xs transition-all flex items-center space-x-2 shrink-0 cursor-pointer whitespace-nowrap"
        >
                ...
                {orders.map((ord) => (
                  <tr key={ord.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-4 px-4 whitespace-nowrap">
                      <span className="font-mono font-black text-blue-600 text-sm block">{ord.order_code}</span>
                    ...
                    <td className="py-4 px-4 whitespace-nowrap">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-extrabold border inline-block ${getStatusBadge(ord.order_status)}`}>
                        ● {ord.order_status}
                      </span>
                    </td>
                    ...
                    <td className="py-4 px-4 text-right whitespace-nowrap">
                      <div className="inline-flex items-center justify-end space-x-2">
                        <button
                          ...
                          className="bg-[#0b2341] hover:bg-[#12315a] text-white px-3.5 py-1.5 rounded-xl text-xs font-bold shadow-2xs transition-all cursor-pointer whitespace-nowrap"
                        >
                          🔍 View Details
                        </button>
                        <Link
                          href="/distributor/invoices"
                          className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs px-3 py-1.5 rounded-xl border border-slate-300 transition-all cursor-pointer whitespace-nowrap"
                        >
                          Invoice
                        </Link>
                      </div>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 space-y-6 p-8 relative">
            ...
            <div className="bg-[#f7f6f4] border border-[#e8e6e2] rounded-2xl p-5 space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border inline-block mt-0.5 ${getStatusBadge(selectedOrder.order_status)}`}>
              ...
              {selectedOrder.tracking_number && (
                <div className="pt-2 border-t border-slate-200/80">
                  ...
                  <span className="font-mono font-bold text-blue-700">{selectedOrder.tracking_number}</span>
                </div>
              )}
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {selectedOrder.items?.map((it, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="py-3 px-3 text-right font-black text-blue-700">₹{it.total_price?.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
